...
// FluidSandbox > Phase 9 — 3D SPH Fluid
//
// Crea la escena 06_3D_SPH.unity con FluidSolver3D configurado para SPH:
//   - SpatialHashGrid3D para búsqueda de vecinos eficiente
//   - Densidad, presión y viscosidad SPH en 3D
//   - Misma jaula y OrbitCamera que Phase 8
public static class Phase9SceneSetup
{
    const string ScenePath = "Assets/Scenes/3D/06_3D_SPH.unity";

    [MenuItem("FluidSandbox/Phase 9 — 3D SPH Fluid")]
    static void Setup()
    {
        CreateScene();
        SetupObjects();
        ...
        EditorUtility.DisplayDialog("Fluid Sandbox",
            ...
            "  Scroll       — zoom\n\n" +
            "Presets (clic derecho en FluidSolver3D):\n" +
            "  Preset/Water · Honey · Slime",
            "OK");
    }
    ...
    {
        var rendGO   = EnsureGO("ParticleRenderer");
        var pRenderer = EnsureComp<ParticleRenderer3D>(rendGO);
        pRenderer.particleColor = new Color(0.2f, 0.55f, 1f, 1f);
